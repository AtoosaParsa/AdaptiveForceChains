...
class simulator:
    ...
    def evaluate(indices):
        ...
        kRatio = 10
        K = np.zeros(N)+kRatio*10*indices
        K = K + 10*(1-indices)
        Kw=np.amax(K)
        ...
        while np.amax([np.amax(ax_old),np.amax(ay_old),abs(awy_old)])>alphacut or np.amax([np.amax(ax_old),np.amax(ay_old),awy_old])==0:
            ...
            Fy[ii]=Fy[ii]-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw) #K[ii]*dw
            Fwy=Fwy+np.sum(2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw))
        ...
        ii=np.where(y<(Dn/2))
        dw=2*(y[ii]-Dn[ii]/2)  #Bottom wall
        Fwb[ii]=-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw)

        ii=np.where(x>(Rw-Dn/2))
        dw=2*(x[ii]-(Rw-Dn[ii]/2))  #Right wall
        Fwr[ii]=-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw)

        ii=np.where(y>(Ly-Dn/2))
        dw=2*(y[ii]-(Ly-Dn[ii]/2))  #Top wall
        Fwt[ii]=-2*Kw*np.multiply(np.divide(K[ii],(Kw+K[ii])),dw)
    ...
    def showPackingWithContacts(indices, save=0):
            ...
            for mm in range(nn+1, N+1):
                ...
                if (abs(dy)<=Dnm):
                    ...
                    if (dnm<Dnm**2):
                        dnm=np.sqrt(dnm)
                        F=-2*K[nn-1]*K[mm-1]/(K[nn-1]+K[mm-1])*(Dnm/dnm-1)
                        plt.plot([x[mm-1], x[nn-1]],[y[mm-1], y[nn-1]], color='black', linewidth=0.5+10*abs(F))
        ...
        for i in range(0, len(ii)):
            for j in ii[i]:
                plt.plot([x[j], x[j]-Dn[j]/2], [y[j], y[j]], color='black', linewidth=0.5+10*abs(Fwl[j]))
        ...
        for i in range(0, len(ii)):
            for j in ii[i]:
                plt.plot([x[j], x[j]], [y[j], y[j]-Dn[j]/2], color='black', linewidth=0.5+10*abs(Fwb[j]))
        ...
        for i in range(0, len(ii)):
            for j in ii[i]:
                plt.plot([x[j], x[j]+Dn[j]/2], [y[j], y[j]], color='black', linewidth=0.5+10*abs(Fwr[j]))
        ...
        for i in range(0, len(ii)):
            for j in ii[i]:
                plt.plot([x[j], x[j]],[y[j], y[j]+Dn[j]/2], color='black', linewidth=0.5+10*abs(Fwt[j]))
