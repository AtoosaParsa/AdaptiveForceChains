""" 
    ...
    output is force from the middle particle on the bottom on the bottom wall
...
class simulator:
    ...
    def evaluate(input):
        ...
        # returning the force from the middle particle on the bottom wall
        return (abs(Fwb[Nout-1] / Fa))
    ...
    def showPacking(input, save=0):
        ...
        plt.show() 
        if save == 1:
            fig.savefig('configuration.png', bbox_inches='tight', format='png', dpi=600, transparent=True)
    ...
    def showPackingWithContacts(input, name, save=0):
        ...
        if save == 1:
            plt.savefig(f'{name}.png', bbox_inches='tight', format='png', dpi=600, transparent=True)
            plt.show()
            plt.close(fig)
        else:
            plt.show()
