""" 
    ...
    output is force from the even particles on the bottom wall
...
class simulator:
    ...
    def evaluate(input):
        ...
        # returning the force O1 = F2*F4
        return [(abs(Fwb[5]*Fwb[11])**(1./2)) / abs(Fa)]
    ...
    def showPacking(input, save=0):
        ...
        if save == 1:
            plt.savefig('configuration.png', bbox_inches='tight', format='png', dpi=600, transparent=True)
            plt.show()
            plt.close(fig)
        else:
            plt.show()
    ...
    def showPackingWithContacts(input, save=0):
        ...
        if save == 1:
            plt.savefig('C2.png', bbox_inches='tight', format='png', dpi=600, transparent=True)
            plt.show()
            plt.close(fig)
        else:
            plt.show()
